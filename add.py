...
class Add:
    ...
    def __init__(self, master, table, member_dict):
        ...
        self.master.title("Adding new membership")
        ...
        ttk.Label(self.frame_content, text="Address:").grid(row=4, column=0, sticky='w', padx = (50,15),pady = 5)
        ...
        self.entry_address.grid(row=4, column=1, sticky = 'w')
        ...
        ttk.Label(self.main_frame, text="None", font = ("bold", 15)).grid(row=3, column=1, sticky='w')


        self.entry_id = ttk.Entry(self.main_frame)
        self.entry_expense = ttk.Entry(self.main_frame)
        
        self.entry_expense.insert(0, "0.0")
        self.entry_expense.bind("<Button-1>", lambda event: self.expense_clear())


        ttk.Label(self.main_frame, font = ("bold",15)).grid(row=3, column=1, sticky='w')
        ...
        self.add = ttk.Button(self.bottom_frame, text = "Add", command = self.validation).grid(row=0, column=2, sticky = 'e', padx = (10,0), pady = 10)
        self.update = ttk.Button(self.bottom_frame, text = "Update", state = DISABLED).grid(row=0, column=3, sticky = 'w', pady = 10)
    ...
    def expense_clear(self, event = None):
    ...
    def validation(self):
        ...
        if len(errors) == 0:
            ...
            self.table.insert(parent='',index='end',iid=num+1,text='', tags = ('odd',), values=(name, email, phone))
